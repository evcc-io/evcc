...
//go:generate mockgen -package api -destination mock.go github.com/evcc-io/evcc/api Charger,ChargeState,CurrentLimiter,CurrentGetter,PhaseSwitcher,PhaseGetter,Identifier,Meter,EnergyImport,EnergyExport,PhaseCurrents,Vehicle,ChargeRater,Battery,Tariff,BatteryController,Circuit
...
// EnergyImport provides imported energy in kWh
type EnergyImport interface {
	EnergyImport() (float64, error)
}

// EnergyExport provides exported energy in kWh
type EnergyExport interface {
	EnergyExport() (float64, error)
}
