...
func (v *Provider) Climater() (active bool, outsideTemp float64, targetTemp float64, err error) {
	...
	if res, ok := res.(ClimaterResponse); err == nil && ok {
		...
		outsideTemp := Temp2Float(GetInterfaceInt(res.Climater.Status.TemperatureStatusData.OutdoorTemperature.Content, res.Climater.Settings.TargetTemperature.Content))
