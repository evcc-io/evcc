...
export default {
	...
	computed: {
		...
		providerLogins() {
			return this.store.state.auth ? Object.entries(this.store.state.auth.vehicles).map((e) => ( {
				title: e[0],
				loggedIn: e[1].authenticated,
				loginPath: e[1].uri + "/login",
				logoutPath: e[1].uri + "/logout",
			})) : [];
		},
	},
	created: function () {
