...
func (wb *NRGKickBLE) connect() (*bluetooth.Device, error) {
	var res *bluetooth.ScanResult
	if err := wb.adapter.Scan(func(adapter *bluetooth.Adapter, device bluetooth.ScanResult) {
		if device.Address.String() == wb.mac {
			res = &device
			wb.adapter.StopScan()
		}
	}); err != nil {
	...
	if res == nil {
		return nil, fmt.Errorf("device not found: %s", wb.mac)
	}

	dev, err := wb.adapter.Connect(res.Address, bluetooth.ConnectionParams{})
	return &dev, err
}
