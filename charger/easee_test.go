...
func TestEasee_Phases1p3p_scaleUp_noChargerCurrentResent(t *testing.T) {
	...
	// Verify the charger settings endpoint was NOT called
	chargerURI := fmt.Sprintf("%s/chargers/%s/settings", easee.API, chargerID)
	info := httpmock.GetCallCountInfo()
	assert.Equal(t, 0, info["POST "+chargerURI], "scale-up must not POST to charger settings")
}

func TestEasee_Phases1p3p_scaleDown_zeroCurrent_stillResetsDCC(t *testing.T) {
	const siteID = 12345
	const circuitID = 67890
	const chargerID = "TESTTEST"
	...
	e.site = siteID
	e.circuit = circuitID
	// e.current is 0 (zero value — MaxCurrent was never called)
