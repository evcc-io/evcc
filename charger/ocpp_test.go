...
func (suite *ocppTestSuite) TestConnect() {
	...
	c1, err := NewOCPP("test-1", 1, "", "", 0, true, ocppTestConnectTimeout, ocppTestTimeout, "A")
	...
	c2, err := NewOCPP("test-2", 1, "", "", 0, true, ocppTestConnectTimeout, ocppTestTimeout, "A")
	suite.Require().NoError(err)
...
func (suite *ocppTestSuite) TestAutoStart() {
	...
	// 1st charge point- local
	c1, err := NewOCPP("test-3", 1, "", "", 0, false, ocppTestConnectTimeout, ocppTestTimeout, "A")
	suite.Require().NoError(err)
