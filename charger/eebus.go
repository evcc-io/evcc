...
func (c *EEBus) onConnect(ski string, conn ship.Conn) error {
	logx.Trace(c.log, "msg", "!! onConnect invoked", "ski", ski)

	eebusDevice := app.HEMS(server.EEBusInstance.DeviceInfo())
	c.cc = communication.NewConnectionController(logx.NewPrintAdapter(logx.TraceLevel(c.log)), conn, eebusDevice)
...
func (c *EEBus) onDisconnect(ski string) {
	logx.Trace(c.log, "msg", "!! onDisconnect invoked", "ski", ski)
...
func (c *EEBus) dataUpdateHandler(dataType communication.EVDataElementUpdateType, data *communication.EVSEClientDataType) {
	...
	case communication.EVDataElementUpdateUseCaseSelfConsumption:
		...
		if err != nil {
			logx.Error(c.log, "msg", "failed to send current limit data: ", "error", err)
		}
...
func (c *EEBus) Status() (api.ChargeStatus, error) {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! status: no eebus data available yet")
	...
	if !c.connected {
		logx.Trace(c.log, "msg", "!! status: charger reported as disconnected")
...
func (c *EEBus) Enable(enable bool) error {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! enable: no eebus data available yet")
		return err
	}

	if data.EVData.ChargeState == communication.EVChargeStateEnumTypeUnplugged {
		logx.Trace(c.log, "msg", "!! currents: ev reported as unplugged")
	...
	if data.EVData.CommunicationStandard == communication.EVCommunicationStandardEnumTypeUnknown {
		logx.Trace(c.log, "msg", "!! enable: cannot enable or disable as communication standard is not yet known")
...
func (c *EEBus) MaxCurrentMillis(current float64) error {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! currents: no eebus data available yet")
		return err
	}
	...
	if data.EVData.LimitsL1.Min == 0 {
		logx.Trace(c.log, "msg", "!! we did not yet receive min and max currents to validate the call of MaxCurrent, use it as is")
	}

	if current < data.EVData.LimitsL1.Min {
		logx.Trace(c.log, "msg", "!! current value %f is lower than the allowed minimum value %f", current, data.EVData.LimitsL1.Min)
		current = data.EVData.LimitsL1.Min
	}

	if current > data.EVData.LimitsL1.Max {
		logx.Trace(c.log, "msg", "!! current value %f is higher than the allowed maximum value %f", current, data.EVData.LimitsL1.Max)
	...
	logx.Trace(c.log, "msg", "!! currents: returning %f", current)
...
func (c *EEBus) CurrentPower() (float64, error) {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! current power: no eebus data available yet")
		return 0, err
	}

	if data.EVData.ChargeState == communication.EVChargeStateEnumTypeUnplugged {
		logx.Trace(c.log, "msg", "!! current power: ev reported as unplugged")
		return 0, nil
	}

	power := data.EVData.Measurements.PowerL1 + data.EVData.Measurements.PowerL2 + data.EVData.Measurements.PowerL3
	logx.Trace(c.log, "msg", "!! current power: returning %f", power)
...
func (c *EEBus) ChargedEnergy() (float64, error) {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! charged energy: no eebus data available yet")
		return 0, err
	}

	if data.EVData.ChargeState == communication.EVChargeStateEnumTypeUnplugged {
		logx.Trace(c.log, "msg", "!! charged energy: ev reported as unplugged")
		return 0, nil
	}

	energy := data.EVData.Measurements.ChargedEnergy / 1000
	logx.Trace(c.log, "msg", "!! charged energy: returning %f", energy)
...
func (c *EEBus) Currents() (float64, float64, float64, error) {
	...
	logx.Trace(c.log, "msg", "!! currents: returning %f, %f, %f, ", data.EVData.Measurements.CurrentL1, data.EVData.Measurements.CurrentL2, data.EVData.Measurements.CurrentL3)
...
func (c *EEBus) Identify() (string, error) {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! identify: no eebus data available yet")
		return "", err
	}

	if !c.connected {
		logx.Trace(c.log, "msg", "!! identify: charger reported as disconnected")
		return "", nil
	}
	...
	if data.EVData.ChargeState == communication.EVChargeStateEnumTypeUnplugged || data.EVData.ChargeState == communication.EVChargeStateEnumTypeUnknown {
		logx.Trace(c.log, "msg", "!! identify: ev reported as unplugged or unknown")
		return "", nil
	}

	if len(data.EVData.Identification) > 0 {
		logx.Trace(c.log, "msg", "!! identify: returning %s", data.EVData.Identification)
	...
	if data.EVData.CommunicationStandard == communication.EVCommunicationStandardEnumTypeIEC61851 {
		logx.Trace(c.log, "msg", "!! identify: ev communication is IEC61851 which does not support any identification")
		return "", nil
	}

	if time.Since(c.evConnectedTime) < maxIdRequestTimespan {
		logx.Trace(c.log, "msg", "!! identify: returning nothing, retry")
		return "", api.ErrMustRetry
	}

	logx.Trace(c.log, "msg", "!! identify: returning nothing, no more retries")
	return "", nil
}
...
func (c *EEBus) SoC() (float64, error) {
	data, err := c.cc.GetData()
	if err != nil {
		logx.Trace(c.log, "msg", "!! soc: no eebus data available yet")
		return 0, api.ErrMustRetry
	}

	if !data.EVData.UCSoCAvailable || !data.EVData.SoCDataAvailable {
		logx.Trace(c.log, "msg", "!! soc: feature not available")
		return 0, api.ErrNotAvailable
	}

	logx.Trace(c.log, "msg", "!! soc: returning %f", data.EVData.Measurements.SoC)
