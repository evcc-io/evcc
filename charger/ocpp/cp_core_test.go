...
// bootTestConnector registers a connector with a running transaction on a fresh
// charge point, mirroring a charger that was charging before a reboot.
func bootTestConnector(t *testing.T) (*CP, *Connector) {
	t.Helper()

	// the connector talks to the central system on creation
	_, err := Instance()
	require.NoError(t, err)
	...
	conn, err := NewConnector(t.Context(), log, 1, cp, "", Timeout)
	require.NoError(t, err)

	conn.status = &core.StatusNotificationRequest{
		ConnectorId: 1,
...
// TestUnsolicitedBootResetsTransaction covers the charger power-cycle: the
// transaction died with it, so it must be cleared and the cached status
// distrusted until the charge point reports a fresh one.
func TestUnsolicitedBootResetsTransaction(t *testing.T) {
	cp, conn := bootTestConnector(t)
	...
	assert.Equal(t, 0, conn.txnId, "a reboot must clear the transaction")
	assert.True(t, conn.statusStale, "the pre-reboot status must be marked stale")

	<-cp.bootNotificationRequestC
}

// TestSolicitedBootKeepsTransaction ensures a BootNotification evcc solicited
// itself to complete the connection handshake is not mistaken for a reboot -
// clearing there would wipe a still-running transaction.
func TestSolicitedBootKeepsTransaction(t *testing.T) {
	cp, conn := bootTestConnector(t)
	...
	assert.Equal(t, 42, conn.txnId, "a solicited boot must keep the transaction")
	assert.False(t, conn.statusStale, "a solicited boot must not invalidate the status")

	cp.mu.RLock()
	assert.False(t, cp.bootTriggered, "the flag must be consumed by the BootNotification")
	cp.mu.RUnlock()

	<-cp.bootNotificationRequestC
}

// TestDisconnectClearsBootTriggered ensures a trigger the charge point never
// answered cannot leak into the next connection and mask a genuine reboot.
func TestDisconnectClearsBootTriggered(t *testing.T) {
	log := util.NewLogger("test")
	cp := NewChargePoint(log, instance, "test-cp")
	...
	cp.mu.RLock()
	assert.False(t, cp.bootTriggered, "an unanswered trigger must not survive a disconnect")
	cp.mu.RUnlock()
}
