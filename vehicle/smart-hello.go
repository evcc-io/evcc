package vehicle

import (
	"context"
	"fmt"
	"time"

	"github.com/evcc-io/evcc/api"
	"github.com/evcc-io/evcc/util"
	"github.com/evcc-io/evcc/vehicle/smart/hello"
)

// SmartHello is an api.Vehicle implementation for Smart Hello cars
type SmartHello struct {
	*embed
	*hello.Provider
}

func init() {
	registry.AddCtx("smart-hello", NewSmartHelloFromConfig)
}

// NewSmartHelloFromConfig creates a new vehicle
func NewSmartHelloFromConfig(ctx context.Context, other map[string]any) (api.Vehicle, error) {
	cc := struct {
		embed          `mapstructure:",squash"`
		User, Password string
		VIN            string
		Cache          time.Duration
	}{
		Cache: interval,
	}

	if err := util.DecodeOther(other, &cc); err != nil {
		return nil, err
	}

	if cc.User == "" || cc.Password == "" {
		return nil, api.ErrMissingCredentials
	}

	log := util.LoggerFromContext(ctx, "smart-hello").Redact(cc.User, cc.Password, cc.VIN)

	v := &SmartHello{
		embed: &cc.embed,
	}

	identity, err := hello.NewIdentity(log, cc.User, cc.Password)
	if err != nil {
		return v, fmt.Errorf("login failed: %w", err)
	}

	api := hello.NewAPI(log, identity)

	vehicle, err := ensureVehicleEx(cc.VIN, api.Vehicles, func(v hello.Vehicle) (string, error) {
		return v.VIN, nil
	})
	if err != nil {
		return v, err
	}

	api.SetSeries(vehicle.SeriesCodeVs)

	v.Provider = hello.NewProvider(log, api, vehicle.VIN, cc.Cache)

	return v, nil
}
