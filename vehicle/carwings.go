...
func NewCarWingsFromConfig(other map[string]interface{}) (api.Vehicle, error) {
	...
	v.statusG = provider.NewCached[carwings.BatteryStatus](func() (carwings.BatteryStatus, error) {
		v.wg.Wait() // initial connect
		return v.status()
	}, cc.Cache).Get

	v.climateG = provider.NewCached[carwings.ClimateStatus](func() (carwings.ClimateStatus, error) {
		v.wg.Wait() // initial connect
