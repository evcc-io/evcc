...
func TestBuildReadConfig_BlockMode(t *testing.T) {
	cfg, err := buildReadConfig(0xF7, 0x8943, 2, &blockread.Block{Register: 0x891C, Count: 125})
	require.NoError(t, err)
...
func TestBuildReadConfig_RejectsRegisterOutsideBlock(t *testing.T) {
	// before block start
	_, err := buildReadConfig(0xF7, 0x8900, 2, &blockread.Block{Register: 0x891C, Count: 125})
	require.Error(t, err)

	// past block end (0x891C+125 = 0x8999; 0x8998+2 overruns)
	_, err = buildReadConfig(0xF7, 0x8998, 2, &blockread.Block{Register: 0x891C, Count: 125})
	require.Error(t, err)
}
