...
func (c *EEBus) dataUpdateLimit() {
	limit, err := c.uc.CsLPC.ConsumptionLimit()
	if err != nil {
...
func (c *EEBus) writeApprovalRequired() {
	for msg, limit := range c.uc.CsLPC.PendingConsumptionLimits() {
		c.log.DEBUG.Println("CS LPC PendingConsumptionLimit:", msg, limit)
		c.uc.CsLPC.ApproveOrDenyConsumptionLimit(msg, true, "")
...
func (c *EEBus) dataUpdateFailsafeConsumptionActivePowerLimit() {
	limit, _, err := c.uc.CsLPC.FailsafeConsumptionActivePowerLimit()
	if err != nil {
...
func (c *EEBus) dataUpdateFailsafeDurationMinimum() {
	duration, _, err := c.uc.CsLPC.FailsafeDurationMinimum()
	if err != nil {
