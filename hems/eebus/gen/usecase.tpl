...
func (c *EEBus) handle{{.Uc}}() error {
	...
	if heartbeatErr != nil && c.{{.uc}}Status != StatusFailsafe {
		...
		c.set{{.Uc}}StatusAndLimit(StatusFailsafe, c.failsafe{{.Uc}}Limit, true)
	...
	case StatusUnlimited:
		...
		if c.{{.uc}}Limit != nil && c.{{.uc}}Limit.IsActive {
			...
			c.set{{.Uc}}StatusAndLimit(StatusLimited, c.{{.uc}}Limit.Value, true)
		}
	...
	case StatusLimited:
		...
		if !c.{{.uc}}Limit.IsActive {
			...
			c.set{{.Uc}}StatusAndLimit(StatusUnlimited, 0, false)
			break
		}
