...
func TestSqliteTimestamp(t *testing.T) {
	...
	persist(entity, clock.Now(), 0, 0, nil, nil)
...
func TestQueryEnergyUTCFilter(t *testing.T) {
	...
	require.NoError(t, persist(e, base, 0, 1, nil, nil))
	require.NoError(t, persist(e, base.Add(time.Hour), 0, 2, nil, nil))
...
func TestQueryEnergyGrouped(t *testing.T) {
	...
	require.NoError(t, persist(e1, base, 1, 0, nil, nil))
	require.NoError(t, persist(e2, base, 2, 0, nil, nil))
	require.NoError(t, persist(e1, base.Add(time.Hour), 3, 0, nil, nil))
	require.NoError(t, persist(e2, base.Add(time.Hour), 4, 0, nil, nil))
...
func TestQueryEnergyMultipleSeries(t *testing.T) {
	...
	for i := range 2 {
		...
		require.NoError(t, persist(eGrid, ts, float64(1+i), 0, nil, nil))
		require.NoError(t, persist(ePv1, ts, 0, float64(10+i), nil, nil))
		require.NoError(t, persist(ePv2, ts, 0, float64(20+i), nil, nil))
	}
...
func TestUpdateProfile(t *testing.T) {
	...
	// day 2:  96 ... 181
	for i := range 4 * 2 * 24 {
		persist(entity, clock.Now(), float64(i), float64(i), nil, nil)
