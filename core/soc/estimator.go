	...
	if chargePower > 0 {
		...
		if vr, ok := s.vehicle.(api.VehicleFinishTimer); ok {
			...
			if !errors.Is(err, api.ErrNotAvailable) {
				s.log.Warn("updating remaining time failed: %v", err)
			}
		}
...
func (s *Estimator) SoC(chargedEnergy float64) (float64, error) {
	...
	if charger, ok := s.charger.(api.Battery); ok {
		...
		if err == nil || !errors.Is(err, api.ErrNotAvailable) {
			if err != nil {
				...
				f = s.prevSoc
				s.log.Warn("vehicle soc (charger): %v (ignored by estimator)", err)
			}
	...
	if fetchedSoC == nil {
		...
		if err != nil {
			...
			// recover from temporary api errors
			f = s.prevSoc
			s.log.Warn("vehicle soc: %v (ignored by estimator)", err)
		}
