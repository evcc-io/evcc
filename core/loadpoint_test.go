...
func TestUpdatePowerZero(t *testing.T) {
	...
	for _, tc := range tc {
		...
		lp.Mode = tc.mode
		lp.Update(0, false, false, false) // sitePower false,0
...
func TestDisableAndEnableAtTargetSoc(t *testing.T) {
	...
	lp.Update(500, false, false, false)
	...
	charger.EXPECT().Enabled().Return(lp.enabled, nil)
	lp.Update(-5000, false, false, false)
...
func TestSetModeAndSocAtDisconnect(t *testing.T) {
	...
	charger.EXPECT().Enable(false).Return(nil)
	lp.Update(-3000, false, false, false)
