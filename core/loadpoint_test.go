...
func TestUpdatePowerZero(t *testing.T) {
	...
	for _, tc := range tc {
		...
		lp.mode = tc.mode
		lp.Update(0, nil, false, false, false, 0, 0, nil, nil) // false,sitePower false,0
	...
	for _, status := range []api.ChargeStatus{api.StatusB, api.StatusC} {
		for _, tc := range tc {
			...
			for step, se := range tc.series {
				...
				current := lp.pvMaxCurrent(api.ModePV, se.site, false, false, false, 0)
...
func TestPVHysteresisForStatusOtherThanC(t *testing.T) {
	...
	current := lp.pvMaxCurrent(api.ModePV, sitePower, false, false, false, 0)
...
func TestDisableAndEnableAtTargetSoc(t *testing.T) {
	...
	lp.Update(500, nil, false, false, false, 0, 0, nil, nil)
	ctrl.Finish()
...
func TestSetModeAndSocAtDisconnect(t *testing.T) {
	...
	charger.EXPECT().Enable(false).Return(nil)
	lp.Update(-300, nil, false, false, false, 0, 0, nil, nil)
...
func TestChargedEnergyAtDisconnect(t *testing.T) {
	...
	charger.EXPECT().Enabled().Return(lp.enabled, nil)
	charger.EXPECT().Status().Return(api.StatusC, nil)
	lp.Update(-1, nil, false, false, false, 0, 0, nil, nil)
...
func TestPVHysteresisAfterPhaseSwitch(t *testing.T) {
	...
	for _, tc := range tc {
		...
		for step, se := range tc.series {
			clck.Set(start.Add(se.delay))
			assert.Equal(t, se.current, lp.pvMaxCurrent(api.ModePV, se.site, false, false, false, 0), step)
		}
