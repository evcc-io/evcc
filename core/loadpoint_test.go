...
func TestKeepEnabledBecauseOfClimater(t *testing.T) {
	...
	cts.lp.enabled = true
	cts.lp.offeredCurrent = minA
	cts.lp.status = api.StatusC

	t.Log("charging below soc target")
	cts.vehicle.EXPECT().Soc().Return(75.0, nil)
	cts.charger.EXPECT().Status().Return(api.StatusC, nil)
	cts.charger.EXPECT().Enabled().Return(cts.lp.enabled, nil)
	cts.charger.EXPECT().MaxCurrent(int64(maxA)).Return(nil)
	cts.lp.Update(500, 0, nil, nil, false, false, 0, nil, nil)

	t.Log("charging above target - climater keeps charger active")
	cts.clock.Add(15 * time.Minute)
	cts.lp.energyMetrics.totalKWh += 3 // simulate energy increase to prevent division by zero in estimator
