...
- multiple [chargers](#charger): Wallbe, Phoenix (includes ESL Walli), go-eCharger, NRGkick (direct Bluetooth or via Connect device), SimpleEVSE, EVSEWifi, KEBA/BMW, openWB, Mobile Charger Connect, FritzDECT and any other charger using scripting
...
- `fritzdect`: build in vehicle chargers connected via AVM [FRITZ!DECT](https://avm.de/produkte/fritzdect/) 200/210 switch (using [AHA interface](https://avm.de/fileadmin/user_upload/Global/Service/Schnittstellen/AHA-HTTP-Interface.pdf))
