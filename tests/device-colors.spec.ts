...
function legendBadge(scope: Locator | Page, label: string): Locator {
	// badge is a <button aria-label={label}>; getByLabel targets it specifically
	// and avoids matching the wrapping focusable <li role="button">. Pages with
	// multiple charts show the same legend in each — pick the first.
	return scope.getByLabel(label, { exact: true }).first();
}

async function expectLegendColor(
	scope: Locator | Page,
	label: string,
	rgb: string
): Promise<void> {
