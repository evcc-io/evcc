...
func (s *HTTPd) RegisterSiteHandlers(site site.API, cache *util.Cache) {
	...
	routes := map[string]route{
		...
		"devicestatus":   {[]string{"GET"}, "/config/devices/{class:[a-z]+}/{id:[0-9.]+}/status", deviceStatusHandler},
		...
		"testmerged":     {[]string{"POST", "OPTIONS"}, "/config/test/{class:[a-z]+}/{id:[0-9.]+}", testConfigHandler},
