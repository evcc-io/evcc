...
func psaToken(vehicleConf config.Named) (*oauth2.Token, error) {
	...
	req, _ := request.New(http.MethodPost, fmt.Sprintf("https://idpcvs.%s/am/oauth2/access_token", td.realm), strings.NewReader(data.Encode()), headers)

	log := util.NewLogger("psatoken")
