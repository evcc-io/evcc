...
func contains(combo []string, typ string) bool {
	for _, v := range combo {
		if v == typ {
			return true
		}
...
func generate(out io.Writer, packageName, functionName string, baseTypes []baseType, dynamicTypes ...dynamicType) error {
	...
	tmpl, err := template.New("gen").Funcs(template.FuncMap{
		"dict":     dict,
		"contains": contains,
		// ordered returns types ordered by name
...
func main() {
	...
	for _, base := range strings.Split(*base, ",") {
		...
		shortType = strings.TrimLeft(shortType, "*")

		bt := baseType{
			Type:      base,
			ShortType: shortType,
			VarName:   strings.ToLower(shortType[:1]) + shortType[1:],
		}
	...
	if err := generate(&buf, *pkg, *function, baseTypes, dynamicTypes...); err != nil {
		fmt.Println(err)
		os.Exit(2)
	}
