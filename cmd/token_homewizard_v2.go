...
var homeWizardV2Cmd = &cobra.Command{
	Use:   "homewizard-v2",
	Short: "Pair with HomeWizard devices (v2 WebSocket-based)",
	Run:   runHomeWizardV2Token,
}

func init() {
	tokenCmd.AddCommand(homeWizardV2Cmd)
	homeWizardV2Cmd.Flags().StringP("name", "n", "evcc", "Product name for pairing")
	homeWizardV2Cmd.Flags().Int("timeout", 10, "Discovery timeout in seconds")
}

func runHomeWizardV2Token(cmd *cobra.Command, args []string) {
	...
	fmt.Println("HomeWizard V2 Device Discovery")
	fmt.Println("===============================")
	fmt.Println()
	...
	printHomeWizardV2Config(paired)
}
...
func printHomeWizardV2Config(devices []pairedDevice) {
	fmt.Println()
	fmt.Println("========================================")
