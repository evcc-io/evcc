...
func TestUnmarshalStatusSNSResponse(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(jsonstr), &res))
	assert.Equal(t, -894, res.StatusSNS.SML.PowerCurr)
	...
	require.NoError(t, json.Unmarshal([]byte(jsonstr), &res))
	assert.Equal(t, -36, res.StatusSNS.SML.PowerCurr)
	assert.Equal(t, 60, res.StatusSNS.SML.Power_L1)
	assert.Equal(t, -111, res.StatusSNS.SML.Power_L2)
	assert.Equal(t, 14, res.StatusSNS.SML.Power_L3)
	assert.Equal(t, 235.3, res.StatusSNS.SML.Voltage_L1)
	assert.Equal(t, 235.7, res.StatusSNS.SML.Voltage_L2)
	assert.Equal(t, 235.7, res.StatusSNS.SML.Voltage_L3)
	assert.Equal(t, 0.98, res.StatusSNS.SML.Current_L1)
	assert.Equal(t, 1.47, res.StatusSNS.SML.Current_L2)
	assert.Equal(t, 1.18, res.StatusSNS.SML.Current_L3)
}
