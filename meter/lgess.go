...
//go:generate decorate -f decorateLgEss -b *LgEss -r api.Meter -t "api.MeterEnergy,TotalEnergy,func() (float64, error)" -t "api.Battery,Soc,func() (float64, error)" -t "api.BatteryController,SetBatteryMode,func(api.BatteryMode) error" -t "api.BatteryCapacity,Capacity,func() float64"
...
func NewLgEssFromConfig(other map[string]interface{}, essType lgpcs.Model) (api.Meter, error) {
	...
	return NewLgEss(cc.URI, cc.Usage, cc.Password, cc.Cache, cc.capacity.Decorator(), cc.battery, essType)
}

// NewLgEss creates an LgEss Meter
func NewLgEss(uri, usage, password string, cache time.Duration, capacity func() float64, battery battery, essType lgpcs.Model) (api.Meter, error) {
	conn, err := lgpcs.GetInstance(uri, password, cache, essType)
	if err != nil {
		return nil, err
	}
...
func (m *LgEss) batteryMode(battery battery) func(api.BatteryMode) error {
	return func(mode api.BatteryMode) error {
		...
		case api.BatteryNormal:
			...
			// if battery is sleeping, wake up with charging
			// todo: check for slepping:  "bat_status": "0"
