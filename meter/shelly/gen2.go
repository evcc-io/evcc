...
type gen2[Status Phases, Data any] struct {
	*gen2base
	emstatus func() (Status, error)
	emdata   func() (Data, error)
}
...
func newGen2(helper *request.Helper, uri, model string, channel int, user, password string, cache time.Duration) (Generation2, error) {
	// Shelly GEN 2+ API
	...
	if user != "" {
		base.Client.Transport = digest.NewTransport(user, password, base.Client.Transport)
	}

	base.switchstatus = util.ResettableCached(apiCall[Gen2SwitchStatus](base, "Switch.GetStatus"), cache)

	var res Gen2Methods
	if err := base.execCmd("Shelly.ListMethods", false, &res); err != nil {
		return nil, err
	}

	base.methods = res.Methods

	if base.hasSwitchEndpoint() {
		return base, nil
	}
...
func (c *gen2base) execCmd(method string, enable bool, res any) error {
	data := &Gen2RpcPost{
		Id:     c.channel,
		On:     enable,
		Src:    "evcc",
...
func (c *gen2base) hasSwitchEndpoint() bool {
